...
- `engine-radiation-uncertainty` and `engine-system-economics` are each published as a standalone JS/TS package (e.g. to npm, or consumed as a git dependency — pick whichever is easiest for a two-person team; npm with a private-by-default or scoped package is simplest).
...
Any change to these shapes needs a version bump on the exporting package and a coordinated PR on the consuming side — this replaces the "coordinate via PR description" note from the single-repo CLAUDE.md, since a cross-repo interface change can no longer be reviewed in one diff.
